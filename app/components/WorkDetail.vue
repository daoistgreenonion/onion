<template>
  <main class="max-w-5xl mx-auto py-12 px-4">
    ...
    <section class="grid grid-cols-1 sm:grid-cols-[4fr,5fr]">
      ...
      <div class="py-3 sm:p-3 flex flex-col justify-between">
        <div>
          <!-- Title -->
        ...
        <div class="flex sm:bg-gray-200 sm:dark:bg-dark2 py-3 sm:p-3 justify-between items-center">
          <!-- Lore link -->
          <div v-if="loreBasePath && work.lore?.length">
            <NuxtLink :to="loreBasePath" class="flex items-center gap-2 text-white w-full p-2 sm:px-5 sm:py-2.5 bg-brand text-gray-100 hover:bg-brand-light">
          ...
          <div v-if="!work.externalLink && firstChapter">
            <NuxtLink
              :to="`${chapterBasePath}/${firstChapter.slug}`" class="flex items-center gap-2 w-full p-2 sm:px-5 sm:py-2.5 bg-brand hover:bg-brand-light transition-colors text-gray-100">
